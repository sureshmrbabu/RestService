...
try{
     ...
     stage('Clone Repo') {
      ...
      stage('Deploy docker'){
              ...
              sh "docker run --name RestService-deploy -d -p 8081:8081 RestService-deploy:${env.BUILD_NUMBER} -v jenkins-data:/var/jenkins_home -v /var/run/docker.sock:/var/run/docker.sock -v $(which docker):/usr/bin/docker"
      }
}catch(e){
